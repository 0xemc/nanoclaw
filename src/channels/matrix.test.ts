...
const syncMode = vi.hoisted(() => ({ value: 'PREPARED' as string }));
  ...
  createClient: (opts: any) => {
    ...
    const client = {
      ...
      startClient: vi.fn(async () => {
        // Fire the configured sync state so connect() resolves or rejects
        const syncHandlers = handlers.get('sync') || [];
        for (const h of syncHandlers) h(syncMode.value, null, {});
      }),
